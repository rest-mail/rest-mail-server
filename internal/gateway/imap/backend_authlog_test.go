...
// TestAuthenticate_LogsMaskedFailure verifies a failed IMAP login emits a
// structured imap_auth_failed event with the attempted username masked (never
...
func TestAuthenticate_LogsMaskedFailure(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "imap_auth_failed") {
		t.Errorf("expected imap_auth_failed event in log, got: %s", out)
	}
