...
func main() {
	...
	// Secure-by-construction (production-only): refuse to boot when the IMAP
	// listeners would serve plaintext where they shouldn't — no TLS keypair to
	// advertise STARTTLS on 143 or bind implicit TLS on 993 (which would also
	// accept LOGIN/AUTHENTICATE before TLS). In development/test (the testbed & e2e
	// default) each finding only warns and boot proceeds unchanged.
	if err := cfg.ValidateListenerSecurity(config.RoleIMAPGateway); err != nil {
